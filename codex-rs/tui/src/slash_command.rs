...
impl SlashCommand {
    ...
    pub fn description(self) -> &'static str {
        match self {
            ...
            SlashCommand::New => "在当前对话中开始新聊天",
            SlashCommand::Init => "创建包含 Codex 项目说明的 AGENTS.md 文件",
            SlashCommand::Compact => "总结当前对话，避免达到上下文长度上限",
            SlashCommand::Review => "审查当前代码改动并查找问题",
            SlashCommand::Rename => "重命名当前任务",
            SlashCommand::Resume => "恢复已保存的聊天",
            SlashCommand::Archive => "归档当前会话并退出",
            SlashCommand::Delete => "永久删除当前会话并退出",
            SlashCommand::Clear => "清空终端并开始新聊天",
            SlashCommand::Fork => "派生当前聊天",
            SlashCommand::App => "在 Codex 桌面应用中继续当前会话",
            SlashCommand::Quit | SlashCommand::Exit => "退出 Codex",
            SlashCommand::Copy => "以 Markdown 格式复制上一条回复",
            SlashCommand::Raw => "切换原始回滚模式，便于在终端中选择和复制文本",
            SlashCommand::Diff => "显示 Git 差异（包括未跟踪文件）",
            SlashCommand::Mention => "引用一个文件",
            SlashCommand::Skills => "使用技能提升 Codex 执行特定任务的能力",
            SlashCommand::Import => "从 Claude Code 导入设置、当前项目和最近聊天",
            SlashCommand::Hooks => "查看和管理生命周期钩子",
            SlashCommand::Status => "显示当前会话配置和令牌用量",
            SlashCommand::Usage => "查看账户用量或使用一次用量限制重置",
            SlashCommand::DebugConfig => "显示配置层级和要求来源以便调试",
            ...
            SlashCommand::ElevateSandbox => "设置提升权限的智能体沙箱",
            SlashCommand::SandboxReadRoot => {
                "允许沙箱读取目录：/sandbox-add-read-dir <绝对路径>"
            }
            SlashCommand::Experimental => "开启或关闭实验性功能",
            SlashCommand::AutoReview => "批准重试一次最近被拒绝的自动审查",
            ...
            SlashCommand::Rollout => "输出会话记录文件路径",
